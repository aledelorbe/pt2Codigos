...
try:
    ...
    database = mysql.connector.connect(host=host, user=username, passwd=password, database=database)
    ...
    for rowEdu, rowEdu0_1, grupoEdu, estado in zip(dfEducaciones.values, dfEducaciones0_1.values, gruposEducaciones.values, estados):
        ...
        for i in range(10):
            ...
            cursor.callproc('sp_insertarEstadoEscolaridad', (estado, descrip, cant, numCluster, porcen))
        ...
        for i in range(10, 46, 1):
            ...
            cursor.callproc('sp_insertarEstadoCancer', (estado, descrip, cant, numCluster, porcen))
            if cursor.rowcount == 1:
                contador += 1
    ...
    for rowEmp, rowEmp0_1, grupoEmp, estado in zip(dfEmpleos.values, dfEmpleos0_1.values, gruposEmpleos.values, estados):
        ...
        for i in range(13):
            ...
            print(estado, descrip, cant, numCluster, porcen)
            # Mandar a llamar el sp que inserta datos de estado con nivel escolar
            cursor.callproc('sp_insertarEstadoOcupacion', (estado, descrip, cant, numCluster, porcen))
            if cursor.rowcount == 1:
                contador += 1
    ...
    if 'conn' in locals() and database.is_connected():
        database.rollback()
